var FGS = {
	...
	parseStartupData: function(data2)
	{
		...
		var pos0 = data2.indexOf('envFlush({')+9;		
		var posA1 = data2.indexOf('Env={')+4;
		
		if(pos0 != 8) {
			var pos1 = data2.indexOf('});', pos0)+1;
			
			try {
				var tmp = JSON.parse(data2.slice(pos0, pos1));
			} catch(e) {
				FGS.dump(FGS.getCurrentTime()+'[R] Error: probably logged out');
				FGS.stopAll();
				return true;
			...
			FGS.userID = tmp.user;
			FGS.post_form_id = tmp.post_form_id;
			FGS.fb_dtsg = tmp.fb_dtsg;
			FGS.userLoc = tmp.locale;
			FGS.ajaxpipe_token = tmp.ajaxpipe_token;
	...
	checkBonuses: function(appID, params, retry)
	{
		...
		if(params.time == 0)
		{
			...
			var feedParams = '__a='+(params.scroll)+'&data={%22show_hidden%22:%22false%22,%22ignore_self%22:%22false%22,%22filter%22:%22'+filterType+'%22,%22scroll_count%22:'+params.scroll+'}&__user='+FGS.userID+'&__adt=2';
		}
		else
		{
			...
			var feedParams = '__a='+(params.scroll+8)+'&data={%22show_hidden%22:%22false%22,%22ignore_self%22:%22false%22,%22filter%22:%22'+filterType+'%22,%22scroll_count%22:'+params.scroll+',%22scroll_position%22:'+(params.scroll*30)+',%22oldestMR%22:'+params.time+',%22oldest%22:'+params.time+'}&__user='+FGS.userID+'&__adt=2';
		}
