		...
		$.ajax({
			...
			success: function(dataStr)
			{
				...
				try
				{
					...
					FGS.cafeworld.Freegifts.Click2(params);
				}
				catch(err)
				{
					FGS.dump(err);
	...
	Click2: function(params, retry)
	{
		...
		$.ajax({
			...
			success: function(dataStr)
			{
				...
				try
				{
					var nextUrl = $('#frmGifts', dataHTML).attr('action');
					var formParam = $('#frmGifts', dataHTML).serialize();
		...
		$.ajax({
			...
			success: function(dataStr)
			{
				...
				try
				{
					...
					if(!url)
					{
						var paramTmp = FGS.findIframeAfterId('#app_content_101539264719', dataStr);
						if(paramTmp == '') throw {message: 'no iframe'}
						var url = paramTmp;
					}
					
					FGS.cafeworld.Requests.Click2(currentType, id, url, params);
				}
				catch(err)
				{
					FGS.dump(err);
			...
			success: function(dataStr)
			{
				...
				try
				{
					...
					if($('form[action*="request_v2_landing_page.php"]', dataHTML).length > 0)
					{
						...
						retryThis(currentType, id, [url, params2], params);
						return;
					}
					...
					if(dataStr.indexOf('This gift has already been accepted') != -1) {
						var error_text = 'This gift has already been accepted';
						FGS.endWithError('limit', currentType, id, error_text);
						return;
					}
					...
					if($('#gift_items', dataHTML).length > 0)
					{
						var titleX = $('#gift_items', dataHTML).find('h1:first').text();
						...
						{
							var gift =  $('#gift_items', dataHTML).find('h1:first').text();
							var from = $('#gift_items', dataHTML).find('h1:first').text();
						}
						...
						info.image = $('#gift_items', dataHTML).find('img:first').attr("longdesc");
						info.title = gift;
						info.text  = from;
						info.time = Math.round(new Date().getTime() / 1000);
		...
		$.ajax({
			...
			success: function(dataStr)
			{
				...
				try
				{
					...
					FGS.cafeworld.Bonuses.Click2(currentType, id, url, params);
				}
				catch(err)
				{
					FGS.dump(err);
			...
			success: function(dataStr)
			{
				...
				try
				{
					...
					if(dataStr.indexOf('please pick a mystery gift as a thank you') != -1)
					{
						...
						retryThis(currentType, id, unescape(newUrl), params, true);
						return;
					}
					
					if(dataStr.indexOf('give you some in return but they can') != -1)
					{
						var newUrl = $('#item_wrapper', dataHTML).find('a:first').attr('href');
					...
					else if($("#item_wrapper",dataHTML).find('.left_cell').find('img').length > 0)
					{
						info.image = $("#item_wrapper",dataHTML).find('.left_cell').find('img').attr("longdesc");
					}
					else
					{
					...
					else if($("#item_wrapper",dataHTML).find('.right_cell').find('h3').length > 0)
					{
						var titleX = $("#item_wrapper",dataHTML).find('.right_cell').find('h3').text();
						...
						info.text  = $("#item_wrapper",dataHTML).find('.right_cell').find('h3').text();
					}
					...
					{
						var gift = $("#item_wrapper",dataHTML).children('.pad:nth-child(1)').text();
						info.text = $("#item_wrapper",dataHTML).children('.pad:nth-child(2)').text();
						info.image = $("#item_wrapper",dataHTML).children('.pad:nth-child(3)').children('img').attr('longdesc');
					}

					if(typeof(info.image) == 'undefined') throw {message: $("#item_wrapper",dataHTML).html()}
